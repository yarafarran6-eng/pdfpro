      ...
      <div style="display:flex;align-items:center;padding:3px 6px;gap:6px;border-bottom:1px solid #eee;position:relative;">
        ...
        <div id="teMenu" style="display:none;position:absolute;top:38px;${ar?'left:0':'right:0'};background:#ffffff;border:1px solid #ccc;border-radius:10px;z-index:9999;min-width:220px;box-shadow:0 8px 24px rgba(0,0,0,0.22);overflow:hidden;">
          ...
          <div style="padding:10px 16px;border-bottom:1px solid #f0f0f0;">
            ...
            <select id="tePageSize" onchange="teApplyPages()" style="width:100%;height:30px;border:1px solid #ddd;border-radius:6px;background:#f9f9f9;font-size:13px;padding:0 6px;">
          ...
          <div style="padding:10px 16px;">
            ...
            <div style="display:flex;align-items:center;gap:6px;">
              ...
              <input type="number" id="tePageCount" value="1" min="1" max="50" style="flex:1;height:32px;border:1px solid #ddd;border-radius:6px;text-align:center;font-size:14px;" onchange="teApplyPages()">
              <button onclick="teChangePages(1)" style="width:32px;height:32px;background:#f0f0f0;border:1px solid #ddd;border-radius:6px;font-size:18px;cursor:pointer;">+</button>
            </div>
            <button onclick="teApplyPages();teToggleMenu()" style="width:100%;margin-top:8px;height:32px;background:#e53935;border:none;border-radius:6px;color:#fff;font-size:13px;font-weight:600;cursor:pointer;">${ar?'تطبيق':'Apply'}</button>
          </div>
        </div>
      </div>
      <div id="teToolbar">
...
async function teSaveAsPDF(){
  ...
  const pageSize=window._tePageSize||'a4';
  ...
  try{
    ...
    const teEd=document.getElementById('teEditor');
    // الفارق بين #teEditor وql-editor (border+padding في Quill)
    const edOff=teEd&&editor?Math.round(editor.getBoundingClientRect().top-teEd.getBoundingClientRect().top):0;
    // أخفِ الـ overlay
    const brk=document.getElementById('tePageBreaks');if(brk)brk.style.display='none';
    // التقط ql-editor (المحتوى الكامل)
    const canvas=await html2canvas(editor,{scale:2,backgroundColor:'#fff',useCORS:true,logging:false});
    if(brk)brk.style.display='';
    if(pf)pf.style.width='50%';
    // فواصل الـ overlay عند ph*i من #teEditor = (ph*i - edOff) من ql-editor top
    const pageHpx=(ph-edOff)*2; // ارتفاع الصفحة الأولى في canvas
    const pageHpxFull=ph*2; // ارتفاع باقي الصفحات
    const pdf=new jsPDF({unit:'mm',format:pageSize,orientation:'portrait'});
    for(let i=0;i<pageCount;i++){
      // الصفحة الأولى: من 0 إلى pageHpx، باقي الصفحات: pageHpxFull لكل منها
      const startY=i===0?0:pageHpx+((i-1)*pageHpxFull);
      const thisH=i===0?pageHpx:pageHpxFull;
      if(startY>=canvas.height&&i>0)break;
      const sliceH=Math.min(thisH,canvas.height-startY);
      const sc=document.createElement('canvas');sc.width=canvas.width;sc.height=pageHpxFull;
      const ctx=sc.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,sc.width,pageHpxFull);
