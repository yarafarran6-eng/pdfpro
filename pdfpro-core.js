...
function teApplyPages(){
  ...
  // احسب الفارق بين #teEditor وql-editor (بسبب border وpadding)
  const qlCont=teEd.querySelector('.ql-container');
  const contOff=qlCont?qlCont.offsetTop:0;
  const edOff=q.root?q.root.offsetTop:0;
  const totalOff=contOff+edOff;
  window._teEdOff=totalOff; // للاستخدام في teSaveAsPDF
  if(count>1){
    const ov=document.createElement('div');
    ov.id='tePageBreaks';
    // top = موضع ql-editor داخل #teEditor لتطابق إحداثيات html2canvas
    ov.style.cssText='position:absolute;top:'+totalOff+'px;left:0;right:0;height:'+(ph*count)+'px;pointer-events:none;z-index:10;';
    const ar2=_lang==='ar';
...
async function teSaveAsPDF(){
  ...
  try{
    const editor=window._teQuill.root;
    const brk=document.getElementById('tePageBreaks');if(brk)brk.style.display='none';
    // التقط .ql-editor (المحتوى الكامل بدون overflow)
    const canvas=await html2canvas(editor,{scale:2,backgroundColor:'#fff',useCORS:true,logging:false});
    if(brk)brk.style.display='';
    if(pf)pf.style.width='50%';
    // الفواصل في .ql-editor عند ph*i من أعلاه = ph*2*i في الـ canvas
    const pageHpx=ph*2;
